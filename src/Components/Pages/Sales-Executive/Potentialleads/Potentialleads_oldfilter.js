...
const Potentialleads = () => {
  ...
  const uniqueOppStatus1 = useMemo(() => {
    const oppStatuses1 = formattedData
      .map((item) => item.opportunity_status1)
      .filter((status) => status && status.trim() !== "");
    return Array.from(new Set(oppStatuses1));
  }, [formattedData]);

  const uniqueOppStatus2 = useMemo(() => {
    const oppStatuses2 = formattedData
      .map((item) => item.opportunity_status2)
      .filter((status) => status && status.trim() !== "");
    return Array.from(new Set(oppStatuses2));
  }, [formattedData]);
  ...
  const filteredData = useMemo(() => {
    return formattedData.filter((item) => {
      ...
      const matchesDestination =
        !filterDestination ||
        (item.destination &&
          item.destination.toLowerCase() === filterDestination.toLowerCase());
    ...
    <div className="salesOpportunitycontainer">
      ...
      <div className={`salesOpportunity ${collapsed ? "collapsed" : ""}`}>
        <div className="potentialleads-table-container">
          ...
          {/* Date range filters with OK button */}
          {showDateRange && (
            <Row className="mb-3">
              <Col md={3}>
          ...
          <Row className="mb-3">
            ...
            <Col md={3}>
              <select
                ...
                onChange={(e) => setFilterOppStatus1(e.target.value)}
              >
                <option value="">All Opportunity Status1</option>
                {uniqueOppStatus1.map((opp1) => (
                  <option key={opp1} value={opp1}>
                    {opp1}
                  </option>
                ))}
              </select>
            ...
            <Col md={3}>
              ...
              >
                ...
                {uniqueOppStatus2.map((opp2) => (
                  <option key={opp2} value={opp2}>
                    {opp2}
                  </option>
                ))}
              </select>
